...
#define QTRMaxcaladd 30
#define QTRMincaladd 40
...
void QTRSensors::calibrate(QTRReadMode mode)
{
  ...
  if(mode == QTRReadMode::USE_EEPROM_cal)
  {
    calibrateOnOrOff(calibrationOn, QTRReadMode::USE_EEPROM_cal);
  }
  else
  {
...
void QTRSensors::calibrateOnOrOff(CalibrationData & calibration, QTRReadMode mode)
{
  ...
  if(mode== QTRReadMode::USE_EEPROM_cal)
  {
    for (uint8_t i = 0; i < _sensorCount; i++)
    {
        calibration.maximum[i] = EEPROM.read(QTRMaxcaladd+i);
        calibration.minimum[i] = EEPROM.read(QTRMincaladd+i);
    }
  }
  else
  {
    ...
    for (uint8_t i = 0; i < _sensorCount; i++)
    {
      ...
      if (minSensorValues[i] > calibration.maximum[i])
      {
        ...
        EEPROM.write(QTRMaxcaladd+i, calibration.maximum[i]);
      }
      ...
      if (maxSensorValues[i] < calibration.minimum[i])
      {
        ...
        EEPROM.write(QTRMincaladd+i, calibration.minimum[i]);
